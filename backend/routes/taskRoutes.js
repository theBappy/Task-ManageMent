...
router.post(
    ...
    body('title', 'Title is required').notEmpty(), // Validation middleware
    body('status', 'Status must be one of: pending, in-progress, completed')
      .optional()
      .isIn(['pending', 'in-progress', 'completed']), // Validation middleware
    body('dueDate', 'Invalid date format').optional().isISO8601(), // Validation middleware
    async (req, res) => {  // Route handler function
